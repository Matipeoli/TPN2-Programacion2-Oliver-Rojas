...
async function listar(req, res) {
  const filas = await sedeService.listar();
  return enviarRespuesta(res, 200, 'ok', filas);
}

async function crear(req, res) {
  const datos = await sedeService.crear(req.body);
  return enviarRespuesta(res, 201, 'ok', datos);
}

async function actualizar(req, res) {
  const datos = await sedeService.actualizar(req.params.id, req.body);
  return enviarRespuesta(res, 200, 'ok', datos);
}

async function eliminar(req, res) {
  const datos = await sedeService.eliminar(req.params.id);
  return enviarRespuesta(res, 200, 'ok', datos);
}
